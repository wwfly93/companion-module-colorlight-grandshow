...
export function parseJson(instance, data) {
	...
	instance.log('info', 'parseJson： nIndex = ' + String(nIndex))
	if (nIndex > 0) {
		...
		instance.setVariableValues({ currentBrightness: brightValue })
	}
	...
	if (nIndex > 0) {
		...
		if (black == 'black') instance.setVariableValues({ Black: true })
		else if (black == 'unBlack') instance.setVariableValues({ Black: false })
	}
}
